...
from app.models import Video, VideoSlice, ProcessingTask
...
@shared_task(bind=True, ignore_result=False, name='app.tasks.video_tasks.extract_slice_audio')
def extract_slice_audio(self, video_id: str, project_id: int, user_id: int, video_minio_path: str, slice_id: int, create_processing_task: bool = True, trigger_srt_after_audio: bool = False) -> Dict[str, Any]:
    """Extract audio from video slice using ffmpeg"""
            ...
            with get_sync_db() as db:
                ...
                if not task:
                    ...
                    task = ProcessingTask(
                        ...
                        input_data={"video_minio_path": video_minio_path, "slice_id": slice_id},
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
                ...
                _update_task_status(celery_task_id, ProcessingTaskStatus.RUNNING, 70, "extracting audio", video_id=video_id)
                self.update_state(state='PROGRESS', meta={'progress': 70, 'stage': ProcessingStage.EXTRACT_AUDIO, 'message': 'extracting audio'})
                ...
                result = run_async(
                    audio_processor.extract_audio_from_video(
                        ...
                        user_id=user_id,
                        custom_filename=f"{video_id}_slice_{slice_id}"
                    )
                )
                ...
                try:
                    ...
                    if converted_audio_path != str(audio_temp_path):
                        ...
                        if audio_url:
                            ...
                            logger.info(f"切片音频采样率已转换并重新上传: {audio_url}")
                        else:
                            logger.error("转换后的音频文件上传失败")
                            raise Exception("转换后的音频文件上传失败")
                except Exception as e:
                    logger.error(f"切片音频采样率检查/转换失败: {str(e)}")
                    # 不中断整个流程，继续使用原始音频
        ...
        if result.get('success'):
                ...
                with get_sync_db() as db:
                    ...
                    if task:
                        state_manager.update_task_status_sync(
                            ...
                            progress=100,
                            message="切片Audio Extraction Completed",
                            output_data={
                                'audio_filename': result['audio_filename'],
                        ...
                        _update_task_status(celery_task_id, ProcessingTaskStatus.SUCCESS, 100, "切片Audio Extraction Completed", video_id=video_id)
            except Exception as e:
                print(f"状态更新失败: {e}")
                # 回退到原来的状态更新方式
                try:
            ...
            self.update_state(state='SUCCESS', meta={'progress': 100, 'stage': ProcessingStage.EXTRACT_AUDIO, 'message': '切片Audio Extraction Completed'})
                ...
                with get_sync_db() as db:
                    ...
                    if slice_record:
                        ...
                        if result.get('minio_path'):
                            slice_record.audio_url = result['minio_path']
                            slice_record.audio_processing_status = "completed"
                            slice_record.audio_progress = 100
                            print(f"设置切片音频路径: slice_id={slice_id}, audio_url={result['minio_path']}")
                        else:
                            slice_record.audio_processing_status = "failed"
                            slice_record.audio_error_message = "无法获取有效的音频文件路径"
                        
                        db.commit()
                        print(f"已更新切片音频状态: slice_id={slice_id}")
            except Exception as e:
                print(f"更新切片音频路径失败: {e}")
                # 更新切片状态为失败
                try:
                    async def _update_slice_status():
                        async with AsyncSessionLocal() as db:
                            stmt = select(VideoSlice).where(VideoSlice.id == slice_id)
                            slice_result = await db.execute(stmt)
                            video_slice = slice_result.scalar_one_or_none()
                            
                            if video_slice:
                                video_slice.audio_processing_status = "failed"
                                await db.commit()
                                print(f"已更新切片音频状态为失败: slice_id={slice_id}")
                    
                    run_async(_update_slice_status())
                except Exception as status_error:
                    print(f"更新切片状态失败: {status_error}")
            ...
            if trigger_srt_after_audio:
                try:
                    ...
                    print(f"切片音频提取成功，将在 {delay:.1f} 秒后触发SRT生成任务: slice_id={slice_id}")
                    time.sleep(delay)
            ...
            try:
                async def _update_slice_status():
                    async with AsyncSessionLocal() as db:
                        ...
                        if video_slice:
                            video_slice.audio_processing_status = "failed"
                            video_slice.audio_error_message = error_msg
                            await db.commit()
                            print(f"已更新切片音频状态为失败: slice_id={slice_id}, error={error_msg}")
                
                run_async(_update_slice_status())
            except Exception as slice_error:
                print(f"更新切片状态失败: {slice_error}")
