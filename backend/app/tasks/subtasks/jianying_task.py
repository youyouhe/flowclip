...
# 定义Jianying特效数组 - 使用中文名称
JIANYING_OPEN_EFFECTS = [
    {"type": "爆炸开幕", "params": None},
    {"type": "渐显", "params": None},
    {"type": "模糊渐显", "params": None},
    {"type": "分屏展开", "params": None},
    {"type": "擦除展开", "params": None}
]

JIANYING_CLOSE_EFFECTS = [
    {"type": "模糊渐隐", "params": None},
    {"type": "横向闭合", "params": None},
    {"type": "渐隐", "params": None},
    {"type": "闭幕", "params": None},
    {"type": "缩放闭幕", "params": None}
]

def _get_proxy_url(resource_path: str) -> str:
    """生成资源的签名URL，供Jianying服务器访问"""
...
@shared_task(bind=True, name='app.tasks.video_tasks.export_slice_to_jianying')
def export_slice_to_jianying(self, slice_id: int, draft_folder: str, user_id: int = None) -> Dict[str, Any]:
    """导出切片到Jianying的Celery任务"""
    ...
    try:
        ...
        _update_task_status(celery_task_id, ProcessingTaskStatus.RUNNING, 5, "开始Jianying导出任务")
        self.update_state(state='PROGRESS', meta={'progress': 5, 'stage': ProcessingStage.JIANYING_EXPORT, 'message': '开始Jianying导出任务'})
        ...
        with get_sync_db() as db:
            ...
            # 添加调试信息
            print(f"DEBUG: 开始Jianying导出任务 - 切片ID: {slice_id}, 类型: {slice_obj.type}")

            # 更新切片的Jianying状态为处理中
            slice_obj.jianying_status = "processing"
            slice_obj.jianying_task_id = celery_task_id
            ...
            _update_task_status(celery_task_id, ProcessingTaskStatus.RUNNING, 10, "创建Jianying草稿")
            self.update_state(state='PROGRESS', meta={'progress': 10, 'stage': ProcessingStage.JIANYING_EXPORT, 'message': '创建Jianying草稿'})

            # 创建草稿
            try:
                draft_result = asyncio.run(jianying_service.create_draft(max_retries=3))
            ...
            self.update_state(state='PROGRESS', meta={'progress': 20, 'stage': ProcessingStage.JIANYING_EXPORT, 'message': '处理视频内容'})
            ...
            if slice_obj.type == "fragment":
                ...
                import random
                open_effect = random.choice(JIANYING_OPEN_EFFECTS)
                close_effect = random.choice(JIANYING_CLOSE_EFFECTS)
                ...
                for i, sub_slice in enumerate(sub_slices):
                    try:
                        ...
                        self.update_state(state='PROGRESS', meta={'progress': progress, 'stage': ProcessingStage.JIANYING_EXPORT, 'message': message})
                        ...
                        open_effect_result = asyncio.run(jianying_service.add_effect(**effect_kwargs))

                        # 添加彩虹渐变特效 (从打开特效结束后持续到子切片结束前3秒)
                        print(f"DEBUG: 添加彩虹渐变特效 - 时间轴起始: {current_time + 3}秒, 时间轴结束: {current_time + sub_slice.duration - 3}秒")
                        rainbow_effect_result = asyncio.run(jianying_service.add_effect(
                            draft_id=draft_id,
                            effect_type="彩虹渐变",
                        ...
                        if proxy_audio_url:
                            audio_result = asyncio.run(jianying_service.add_audio(
                                draft_id=draft_id,
                        ...
                        close_effect_result = asyncio.run(jianying_service.add_effect(**close_effect_kwargs))
                        ...
                        if sub_slice.cover_title:
                            ...
                            text_result = asyncio.run(jianying_service.add_text(
                                draft_id=draft_id,
                        ...
                        video_result = asyncio.run(jianying_service.add_video(
                            draft_id=draft_id,
                ...
                if ending_video_url:
                    ...
                    if proxy_ending_video_url:
                        print(f"DEBUG: 添加渐显特效 - 时间轴起始: {current_time}秒, 时间轴结束: {current_time + 3}秒")
                        open_effect_result = asyncio.run(jianying_service.add_effect(
                            draft_id=draft_id,
                            effect_type="渐显",
                            start=current_time,
                            end=current_time + 3,
                        ...
                        ending_video_result = asyncio.run(jianying_service.add_video(
                            draft_id=draft_id,
                ...
                try:
                    ...
                    # 添加彩虹渐变特效 (从打开特效结束后持续到视频结束前3秒)
                    print(f"DEBUG: 添加彩虹渐变特效 - 时间轴起始: 3秒, 时间轴结束: {slice_obj.duration - 3}秒")
                    rainbow_effect_result = asyncio.run(jianying_service.add_effect(
                        draft_id=draft_id,
                        effect_type="彩虹渐变",
                        start=3,
            ...
            self.update_state(state='PROGRESS', meta={'progress': 80, 'stage': ProcessingStage.JIANYING_EXPORT, 'message': '添加文本和字幕'})
            ...
            self.update_state(state='PROGRESS', meta={'progress': 90, 'stage': ProcessingStage.JIANYING_EXPORT, 'message': '保存草稿'})

            # 保存草稿（异步轮询）
            try:
                print(f"DEBUG: 开始异步保存草稿并等待结果")
                _update_task_status(celery_task_id, ProcessingTaskStatus.RUNNING, 90, "保存草稿并等待结果")
                self.update_state(state='PROGRESS', meta={'progress': 90, 'stage': ProcessingStage.JIANYING_EXPORT, 'message': '保存草稿并等待结果'})

                save_result = asyncio.run(jianying_service.save_draft_and_wait_result(
                    draft_id=draft_id,
                ...
                if save_result.get("success"):
                    ...
                    self.update_state(state='SUCCESS', meta={'progress': 100, 'stage': ProcessingStage.JIANYING_EXPORT, 'message': 'Jianying导出完成'})
                ...
                print(f"保存草稿失败: {e}")
                # 更新切片状态为失败
                slice_obj.jianying_status = "failed"
                slice_obj.jianying_error_message = str(e)
                db.commit()
        ...
        print(f"DEBUG: Jianying导出任务执行失败 - 切片ID: {slice_id}, 错误: {error_msg}")
        print(f"详细错误栈: {error_details}")

        # 更新切片的Jianying导出状态为失败
        try:
            with get_sync_db() as db:
                slice_obj = db.get(VideoSlice, slice_id)
                if slice_obj:
                    slice_obj.jianying_status = "failed"
                    slice_obj.jianying_error_message = error_msg
                    db.commit()
