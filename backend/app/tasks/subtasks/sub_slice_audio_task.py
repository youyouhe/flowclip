    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if video_minio_path.startswith(bucket_prefix):
                ...
                _update_task_status(celery_task_id, ProcessingTaskStatus.RUNNING, 30, "The video file is being downloaded.")
        ...
        if result.get('success'):
                ...
                with get_sync_db() as db:
                    ...
                    if task:
                        state_manager.update_task_status_sync(
                            ...
                            progress=100,
                            message="子切片Audio Extraction Completed",
                            output_data={
                        ...
                        _update_task_status(celery_task_id, ProcessingTaskStatus.SUCCESS, 100, "子切片Audio Extraction Completed")
            except Exception as e:
                print(f"状态更新失败: {e}")
                # 回退到原来的状态更新方式
                try:
            ...
            self.update_state(state='SUCCESS', meta={'progress': 100, 'stage': ProcessingStage.EXTRACT_AUDIO, 'message': '子切片Audio Extraction Completed'})
